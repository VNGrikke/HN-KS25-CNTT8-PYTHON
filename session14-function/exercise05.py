...
def redeem_rewards(records):
    ...
    try:
        points = int(input("Nhap so diem can tieu: "))
...
def appeal_score(records):
    ...
    try:
        points = int(input("Nhap so diem hoan lai: "))
...
def activate_multiplier(records):
    ...
    try:
        multiplier = float(
            input("Nhap he so nhan moi (1.0 - 3.0): ")
        )
...
def grade_assignment(records):
    ...
    try:
        base_points = int(
            input("Nhap so diem goc dat duoc: ")
        )
        ...
        print(
            f"Diem thuc nhan: {earned_points}"
        )
